---
name: bulletin
description: 指定された処理について、開始地点から処理対象までの振る舞いを一本線で追跡するコードリーディング資料「Bulletin解析」(ngbi-*.md) を作成する。30秒で分かる結論・mermaid全体図・コードを追うN地点の構成で出力する。「/bulletin {処理内容}」「/ngbi {処理内容}」「ngbi {処理内容}」「Bulletin解析を作って」「この処理がどこから始まってどこへ届くか追跡して」「処理の流れを一本線で資料化して」といった依頼で使用する。`--code` を付けると、資料からリンクしたコード行にも [ngbi:{資料番号}-{注記番号}] の注記を付ける。
...
7. **`--code` 指定時は、コードに注記を付け、資料末尾に注記一覧を置く**（「コード注記（`--code` 指定時）」）。
...
引数に `--code` が含まれる場合は、`--code` を処理内容から除いて資料を作成する。資料の自己レビューが終わった後、資料からリンクしたコード行へ注記を付け、資料の末尾に注記一覧を置く。`--code` がなければコードは変更しない。
...
| 要素 | 決め方 |
| --- | --- |
| `{資料番号}` | 資料ファイル名の2桁連番（`ngbi-01-...` なら `01`） |
| `{注記番号}` | 2桁連番。資料内でリンクが初めて現れた順に `01` から振る。同じファイルの同じ行へのリンクは同じ番号にする |

対象は、資料内のプロジェクトのソースファイルへのリンクすべて（コードを追うN地点、分岐の該当処理、補足の経路）。

### 付け方

**どのファイルでも行を追加・削除しない。** 行番号が変わると、この資料と既存の ngbi 資料のリンクがずれるため。

| ファイル | 付ける位置 | 例 |
| --- | --- | --- |
| `.ts` `.scss` など行コメントがある形式 | リンク行の末尾 | `this.rowClicked.emit(param); // [ngbi:01-02]` |
| `.css` | リンク行の末尾にブロックコメント | `color: red; /* [ngbi:01-07] */` |
| `.html` | リンク行を含む要素の開始タグ `<tag` の直前（同じ行）。開始タグの外の行（`@if` などの制御フロー、テキスト）は、その行のインデントの直後 | `<!-- [ngbi:01-05] --><tr [pRowToggler]="..."` |

- HTMLの属性行（`(click)="..."` など）の行末には書かない。`// ...` は属性やバインドとして解釈され、`<!-- -->` は開始タグの中に書けないため。
- 既に `[ngbi:...]` の注記があるコメントには、新しいコメントを作らず追記する（`// [ngbi:02-03] [ngbi:01-05]`）。同じ開始タグに複数の注記を付ける場合も、1つのコメントにまとめる（`<!-- [ngbi:01-05] [ngbi:01-06] --><tr`）。
- 付けると構文が壊れる行（複数行のテンプレートリテラル・文字列・`{{ }}` の途中など）や、コメントを書けない形式（JSONなど）には付けず、一覧で「注記なし」とする。

### 資料末尾の注記一覧

「Bulletin解析到達点」（追跡不能時は「追跡不能」）の後に置く。

```markdown
## コード注記一覧

- `[ngbi:01-01]` [`(click)` バインド](./src/app/webapp-common/shared/ui-components/data/table/table.component.html#L138)（注記は L125 の `<tr` の手前）
- `[ngbi:01-02]` [`TableComponent.checkClick()`](./src/app/webapp-common/shared/ui-components/data/table/table.component.ts#L566)
- 注記なし [`foo` テンプレート](./src/app/example.component.ts#L40) — 複数行のテンプレートリテラルの途中のため
```

- 注記番号順に、本文と同じリンクラベル・リンク先で並べる。
- HTMLで、注記を付けた行とリンク先の行が異なる場合だけ「（注記は L{行} の `<{tag}` の手前）」を添える。

### 注記後の確認

- 注記前後で、各ファイルの行数（`wc -l`）が変わっていない。
- `grep -rno "\[ngbi:{資料番号}-[0-9]*\]" src | wc -l` の件数が、一覧の注記ありの件数と一致する。
- 注記を付けたファイルの `git diff` に、注記以外の変更が混ざっていない（作業前からあった差分は除く）。
...
- [ ] `--code` 指定時: 全ソースリンクが注記一覧に載り（注記なしは理由付き）、行数が変わっておらず、HTMLの属性行の末尾に注記を書いていないか
