...
export class ExperimentInfoExecutionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.formDataSubscription = this.executionInfo$.subscribe(formData => {
      // Task switching reuses this component, so subscription updates may not trigger rendering.
      // Mark the component for checking after synchronizing the execution data.
      this.formData = formData;
      const requirementsState = createExecutionRequirementsState(formData, this.selectedRequirement);
      this.requirementsOptions = requirementsState.options;
      this.selectedRequirement = requirementsState.selected;
      this.editableRequirements = requirementsState.editable;
      this.resetRequirementToolTip = requirementsState.resetTooltip;
