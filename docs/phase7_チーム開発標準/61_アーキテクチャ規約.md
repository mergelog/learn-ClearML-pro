...
`Angular Web`（自作分）、`ml`、`services`、`tools`、`scripts` を対象とする。
...
`Angular Web` は ClearML Web を fork した作業ツリーである。自作分は
...
汎用の関数（utility）は、使う feature の中に置く。`data-catalog` の `query` / `export` / `download` が
その形であり、どれも feature の語彙に依存するため、外へ出すと呼ぶ側の文脈を引数で持ち回ることになる。

2つ以上の feature が同じ関数を要求したときに限り `shared` へ移す。
移す先は新しいファイルにし、`shared/utils/url.ts` へ足さない。このファイルは `webapp-common` の7か所から
参照されており、取り込み側の要求で存在する。自作の判断を混ぜると、upstream の更新と競合する。
...
| 画面と Python 側が指す名前の一致 | `pnpm web:contract` |
...
* `web-boundaries.json`（依存の規則と既知の違反）
...
| ------- | ---------- | ------- |
| 0.1 | 2026-09-12 | 初版 |
| 0.2 | 2026-09-12 | 60・62 を未着手扱いから参照へ変更。§4.9 の `web:contract` の説明を実体に合わせた |
| 0.3 | 2026-09-12 | §4.6 に utility の置き場所を追加 |
